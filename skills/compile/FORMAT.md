...
<repo root>/
  ...
  raw/                         # raw substrate + external enrichment (sibling of briefs/, see below)
    ...
    run-<stamp>.report.md      # per-run report for a claude-code compile (frozen; see below)
```
...
The `## Next steps` section is the backlog, and it lives in the report deliberately — every next step sits under the source that raised it, so its provenance is intrinsic, not a flat central list that loses where each item came from. Items are GFM task-list bullets so a reader can check one off: `- [ ] <a research thread to chase or a thing to do> (src: <source-id> · <date> · [[brief]])`, `- [x] …` once done. A leading `⭐ ` right after the checkbox flags an item as a follow-up worth acting on — `- [ ] ⭐ <thread> (src: …)` — orthogonal to done (an item can be flagged and/or done), and it survives recompile because the report is frozen. The flag is what `/clearvoid:recall`'s flagged mode collects (the starred-and-open items, plus their briefs and reports) to hand an agent. One list — no follow-ups/actions split: an item backed by a brief just carries its `[[brief]]` link. **Grounded only**: a concrete next source, a thread the source actually raised, or a specific implication — never generic "explore X further" filler; a source that raises nothing grounded gets no section. The derived backlog view (recall's backlog and flagged modes, the desktop app) scans these sections across every report and groups them by source. The desktop app can toggle an item done, flag it as a follow-up, or delete it (writing back to the report); compile never revisits a frozen url report, so done-ness and flags are the human's to express (a `research`/`md` report is re-written wholesale on re-run, so express durable state on a source you won't re-compile). Sessions (`claude-code:`) have no per-source report and the per-run report carries no `## Next steps`, so sessions emit no backlog items — their threads surface as briefs. `md:` and `chat:` sources **do** write reports, so their `## Next steps` join the backlog like any other.

**The per-run report** is the session stream's member of the family: `raw/run-<stamp>.report.md` (stamp `YYYY-MM-DDTHHMMSS`, local time), written by a `claude-code` compile that created or updated at least one brief — a run that changed nothing writes no file. One report covers the whole batch of sessions a run folded in, so a multi-brief compile is reviewed from one file instead of N diffs. Frontmatter: `report_of: run:<stamp>`, `title:`, `generated:`. Sections: `## Summary` (what the batch worked through — the run's reading view) and `## Briefs updated` (the review index: one `[[slug]]` entry per brief created/updated with what changed and why, every framing change called out old → new) always present; `## Threads noticed` (recurring threads seen but not promoted) best-effort. No `## Next steps` — the backlog stays per-source. Written once and **frozen**: the watermark advances at finalize, the batch never re-queues, and no later run rewrites it. Readers that join reports to sources by `report_of:` ignore `run:` reports by construction (no brief ever cites a `run:` id in `sources:`); they are surfaced by run-oriented viewers instead.
...
- **A framing may carry handling rules, and they bind every consumer.** Beyond steering what to attend to, a framing can state *how its material must be handled*: a register marker ("personal, sensitive — handle with care"), quoting rules ("a line here is one moment's phrasing, never a thesis to cite back at him"), attribution care. These rules bind the compile's Update step **and** every downstream surface that primes with the framing (a briefs-primed chat, recall). Marking a brief's personal register explicitly in its framing is the supported sensitivity mechanism — there is deliberately no separate sensitivity field, for the same reason there is no status field: the framing is prose a human curates, and every consumer is expected to read it. The repo-wide default posture lives in the **briefs charter** (`<briefsRoot>/README.md`, read at SKILL.md step 1; skipped by every brief indexer by convention).
...
- Source ids are namespaced from day one: `claude-code:<sessionId>`, `md:<abs-path>`, `url:<canonical-url>`, `research:<key>`, `chat:<thread-id>`, `chatgpt:<conversationId>` — the schema is source-aware, so each new source drops in without migration. The `url:` id is the canonical URL itself (the same string the watermark uses); it stays resolvable as long as the page exists (and the `raw/` cache keeps a local copy of the extracted substrate regardless). A browser-captured conversation URL (`sources/browser-capture.md`) may be private — resolvable only for its owner — which is fine: the `raw/` copy is the durable substrate and the URL stays the identifier. The `research:` id is the research key (a topic slug, or a slug of the researched URL); its substrate is `raw/<key>.research.md` and it re-folds when re-run (content-hash watermark). The `chat:` id is the thread id; its watermark is the thread's `messageCount` (a monotonic integer), so a thread re-folds when it grows. See the per-source docs under `sources/`.
...
- `units` is a **progress offset** — the count of non-empty JSONL lines compiled through — not a boolean. Sessions grow; the next run renders only lines past the offset (`renderSession --from-line`). (The resume-offset pattern; a visited *flag* would either skip new messages or recompile whole sessions.) Other sources define their own unit in their source module: the `md:` source's unit is a whole file and its watermark is a content hash (`sha256:<hex>`); the `url:` source's unit is a URL and its watermark is the canonical URL itself (a stable string — published content doesn't change, so a compiled URL never auto-re-queues; the exception is a browser-captured conversation URL, which re-queues whenever explicitly named and keeps its capture position in the raw file's `turns:` frontmatter instead of the watermark). `units` is therefore an opaque per-source token, compared by the source, not always a count.
