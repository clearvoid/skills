---
name: recall
description: Load the user's compiled briefs relevant to the current conversation — resolves the current repo's briefs plus every registered root (other repos), scans their frontmatter to build a selection list, and loads only the relevant briefs. Also has a backlog mode that surfaces the next-steps tracked in each source's report, and a flagged mode that pulls the items the user starred plus their briefs and reports for handing to an agent. Use when the user asks to load brief context, asks what their briefs say about something, asks what's on the backlog or what to do/research next, asks for their flagged/starred follow-ups, or before strategy/research/direction work in a repo that has briefs.
user-invocable: true
argument-hint: "[topic to focus the selection, e.g. 'pricing decisions'; 'backlog'/'next steps' for the backlog; 'flagged'/'starred' for the flagged follow-ups]"
...
- **Flagged — load the starred follow-ups for action.** When the directive is a flag word (`flagged`, `starred`, `my flags`, "what have I flagged", "review my flags"), surface the items the user marked with a ⭐ (the follow-ups worth acting on) AND load their full context, so you're ready to start work on them (see Flagged mode below). This is the action cut of the backlog: only flagged-and-open items, plus the briefs and reports behind them.
...
The user flags follow-ups (a leading ⭐ on a next-steps item) while reviewing in the desktop app; this mode pulls those flags together with everything needed to act on them, so a flag-review can flow straight into starting an agent.

1. `node ${CLAUDE_SKILL_DIR}/scripts/resolveRoots.mjs --flagged` — returns `{ scope, mode: "flagged", roots, load }`. `roots` is the in-scope roots that have flagged-and-open items, each `{ name, sources: [{ url, title, reportPath, items: [{text, done, flagged, raw}] }] }` (already filtered to `flagged && !done`). `load` is the full context to read: `{ briefs: [<path>...], reports: [<path>...] }` — the briefs each flagged item links (`[[slug]]`, resolved to paths) and the per-source reports the flagged items live in.
2. **Load the context.** Read every path in `load.briefs` (the current view — stop before any `## Log`, same as default mode) and every path in `load.reports` (in full). This is the deliberate maximum-context load: an agent handed the flags should have the briefs and the source reports behind them, not just the to-do lines.
3. **Surface and tee up.** List the flagged items grouped by root then source (with their `[[brief]]` links and source/date tails), confirm what you loaded (briefs + reports), then you're positioned to act: propose or start the work the flags describe.
4. **Read-only, same as default mode.** Never edit a report from recall — flags are set by the user in the desktop app (or by hand-editing the report).

## Process

1. **Resolve the roots and their briefs.** `node ${CLAUDE_SKILL_DIR}/scripts/resolveRoots.mjs` Returns `{ scope, current, others, available }`. `current` is the repo you're in: a line per brief (slug, title, summary, updated, `collection`, `anchor`, `tags`, `currentViewLines`) read straight from the files. `anchor: true` marks a load-bearing brief (always loaded — see Select/Load); `currentViewLines` is how many leading lines form the current view (everything above the trailing `## Log` history), or `null` if the brief has no log. `others` is the registered repos that the active **context scope** lets in — same per-brief shape. `available` is registered repos the scope left **out** (name + brief count only, no brief lines — out-of-scope briefs are never loaded into the surface). It walks each in-scope root recursively, so briefs nested in collection subfolders are included; `collection` is the brief's folder path relative to the briefs root (`""` for top-level, e.g. `yc-ai` or `ai/agents` for nested ones).
...
- **Read-only — the constraint is on recall, not on briefs.** Don't write, edit, or reorganize briefs *from this skill*: recall is the read path. Briefs themselves are editable substrate — they change via a `/clearvoid:compile` run, a hand-edit, **or an agent editing the file directly when the user asks for it** (e.g. "fix that line", "add X to the brief"). A direct edit is legitimate; it just skips compile's provenance/watermark trail, so prefer a compile run when the source-of-record matters. Don't deflect a "just update the brief" request into running a full compile or telling the user to edit it themselves — make the edit.
